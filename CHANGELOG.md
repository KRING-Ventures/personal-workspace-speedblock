...
- `agent-files/SCHEDULES.md` — fifth scheduled job: a weekly **update check** (Mondays ~09:00). Pulls the framework, compares versions, and if there's a new one, catches up and sends the user a short "what's new". This is the proactive trigger behind the "what's new" rule — without it, a user who rarely opens a session would never learn an update landed. Registered in BOOTSTRAP Phase 5 and self-healed at boot alongside the other jobs.
