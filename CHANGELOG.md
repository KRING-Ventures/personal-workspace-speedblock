...
- `tool-setup.md` — **new.** Step-by-step for the three required apps (plan/tier, link, time, per-app steps), so the person provisioning accounts has enough detail to actually do it. `activation.md` stays one-screen and links here. ChatGPT account flagged as an open item to confirm.
