...
- `tool-setup.md` — **new.** Step-by-step for the three required apps (plan/tier, link, time, per-app steps), so the person provisioning accounts has enough detail to actually do it. `activation.md` stays one-screen and links here. **Ownership made explicit: the venture owns its whole tech stack — accounts and billing stay with them, nothing runs through KRING; KRING only connects the agents.** ChatGPT resolved as the venture's own **ChatGPT Business (Team)** account (chatgpt.com, one seat per person) — the agent runs over it via the **Codex CLI**, not a metered OpenAI API account. Usage-ceiling question (base Business seat vs. credits/higher tier for an always-on agent) flagged as an **Ask Corey** placeholder in `activation-kring.md`.
...
### Changed — Boot-budget + KISS pass on the agent files (WIP, toward 1.1)

The boot bundle (files loaded every session) was overflowing OpenClaw's ~60k-char context budget, so the runtime was truncating — and per-user `MEMORY.md`, loaded last, was getting dropped entirely. `AGENTS.md` alone was 27.8k, over half the budget. Acted on `openclaw doctor` output plus feedback that the file was making agents sound like an audit log instead of a person. Aligned with the official guidance at <https://docs.openclaw.ai/concepts/memory> (keep boot files compact; push detail to on-demand files; never let long-term memory be the file that drops).

- `agent-files/AGENTS.md` — **rewritten leaner: 27.8k → 12.6k (−55%).** No rules dropped — verbose explanation collapsed and detailed how-tos moved to pointers (`runbooks/`, `SCHEDULES.md`, `templates/`, already loaded on demand). The long catch-up and operations-layer prose became short pointers; the permission table, calendar ask-first split, KRING rules, and the 4 AI Commandments are kept in full.
- **The three heavy `## Procedures` → a light `## How you answer`.** `verify-before-stating`, `clear-and-complete-instructions`, and `do-first-then-ask` were Trigger/Steps/Fallback/**Proof** blocks, with a boot-sequence anchor forcing the agent to scan them and emit "Proof" in *every* reply — which forced a source ref on every factual claim and made replies read like an audit log. Replaced with three plain principles (don't state guesses as facts; do it yourself before handing it back; make handed-over instructions followable) and the proof-in-every-output machinery removed. The underlying behaviour is unchanged; the bar is now "don't pass a guess off as certainty," not "cite a source on every sentence."
- `agent-files/EVALS.md` — pass/fail contract updated to match: the verify evals no longer fail an answer for lacking a source ref (only for fabrication / answering without checking); target labels and the "when to run" trigger renamed from the old procedure names.
- `agent-files/SCHEDULES.md` — the weekly **update check** now also runs a context-budget check (`openclaw doctor`) and, on truncation, moves detail to `runbooks/` rather than letting `MEMORY.md` drop. This is the standing guard so the boot bundle can't silently regrow past budget.
- `agent-files/AGENTS.md` — new **Keep the boot bundle lean** section codifies the rule: boot files hold only what's needed in-hand every session; procedures and reference go to `runbooks/` and load on demand.
- `agent-files/runbooks/building-automations.md` — pointer updated to the renamed habit.

**Open for refinement:** depth of the 4 AI Commandments in onboarding; the exact Venture/KRING/User labor split (flagged "needs Corey's eyes" in the design); a separate KRING-as-its-own-venture activation flow (raised, not yet built); a modest trim of `SOUL.md`/`HEARTBEAT.md` if the budget needs more headroom once per-user memory grows.
