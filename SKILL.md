...
description: >
  ...
  receive the venture's intake, deploy one OpenClaw runtime per user (one
  assistant per user), seed each runtime's local working directory from the
  framework's per-user blueprints, set each assistant's name from the intake,
  wire Telegram, confirm the assistants are reachable, and hand the assistant
...
- **Framework-version catch-ups for existing assistants.** Handled by each assistant's session-boot catch-up loop against `CHANGELOG.md` + `agent-files/onboarding/MIGRATIONS/`.
...
For each user, you set up:

1. **The OpenClaw runtime** — a Telegram-reachable OpenClaw instance, pointed at the shared framework (this repo's `agent-files/`), with the assistant's name set from the intake.
2. **The runtime's local working files** — seeded from the framework's per-user blueprints (`IDENTITY`, `USER`, `TOOLS`, `automations/`, empty `MEMORY`, empty `memory/`, `STATE_VERSION`). The blueprints carry `{{FROM_BOOTSTRAP}}` markers — the assistant fills those in during the first conversation.

State lives on the runtime's local filesystem. The assistant *doesn't* push per-user state back to GitHub — GitHub is read-only for the assistant, used only to pull framework templates and updates.

Once the assistants are reachable on Telegram, hand the assistant Telegram handles back to the venture.
...
1. **Verify the intake.** Confirm everything in *Preconditions* is in place. If anything is missing, bounce it back to the venture before starting.
2. **For each user in the intake:**
   - Spin up a new OpenClaw runtime instance.
   - Seed the runtime's local working files from this repo's `agent-files/` per-user blueprints (`IDENTITY.md`, `USER.md`, `TOOLS.md`, `automations/AUTOMATIONS.md`, empty `MEMORY.md`, empty `memory/`, `STATE_VERSION`). Leave `{{FROM_BOOTSTRAP}}` markers in place — the assistant fills them during the first conversation.
   - Set the assistant's `{{AGENT_NAME}}` from the intake.
   - Point the runtime at the shared framework (`KRING-Ventures/personal-workspace-speedblock/agent-files/`) for catch-up reads.
...
- **Framework files** (identical for every assistant, read on demand from this repo): `SOUL.md`, `AGENTS.md`, `HEARTBEAT.md`, `KRING.md`, `MEMORY.md` template, `templates/`, `onboarding/BOOTSTRAP.md`, `onboarding/STATE_VERSION`, `onboarding/MIGRATIONS/`.
- **Per-user blueprints** (seeded into each runtime's local working directory, filled by the assistant during the first conversation): `IDENTITY.md`, `USER.md`, `TOOLS.md`, `automations/AUTOMATIONS.md`.
...
- **`ai-commandments.md`** — The 4 AI Commandments for working with AI agents, plus the must-know vocabulary (branch, pull request, merge, etc.). The agent teaches this in BOOTSTRAP Phase 4 and reinforces it during ongoing work.
- **`onboarding.md`** — The three-step flow in detail (Activate → Build & deploy → Finish onboarding). Read by the venture.

## Version

Current framework version lives in `agent-files/onboarding/STATE_VERSION`. Each deployed assistant tracks its own version locally and catches up via the loop in `agent-files/AGENTS.md`. Shipping a new version: update `CHANGELOG.md`, bump `STATE_VERSION`, tag `v<x.y.z>`, push.
