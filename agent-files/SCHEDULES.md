...
| **Heartbeat check** (`heartbeat`) | Prefiltered | `0 8-18 * * *` (hourly 08:00–18:00) | Run a cheap signal check first (see `runbooks/smart-triggers.md`), then the `HEARTBEAT.md` protocol only when needed. Covers commitment slips, deadlines, calendar-load, and calendar invites awaiting a response (offer accept/decline). General email and meeting prep have their own jobs. | Slack only if useful now |
